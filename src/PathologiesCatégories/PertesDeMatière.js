...
function PertesDeMatière({pathologies}) {
...
const pathologiesPerPage = 10;
  ...
  const currentPathologies = filteredPathologies.filter(pathologie => pathologie.category === "PERTE MATIERE").slice(indexOfFirstPathologie, indexOfLastPathologie);
  ...
  <span className='Path' style={{ color: 'blue' }}>&gt;</span> {/* Utilisation de span pour le symbole ">" */}
  <Link to="/perte de matière" style={{ marginLeft:'10px',color: 'blue', textDecoration: 'none' }}> {t("Menu.PDM")}</Link> {/* Lien vers la page Monument */}
</div>
...
export default PertesDeMatière;