...
function ChromatiqueDépot({pathologies}) {
...
const currentPathologies = filteredPathologies.filter(pathologie => pathologie.category === "CHROMATIQUE").slice(indexOfFirstPathologie, indexOfLastPathologie);
...
const handleSearch = () => {
 ...
 setCurrentPage(1); // Réinitialiser à la première page après la recherche
  ...
  <Link to="/acceuil" style={{ color: 'blue', textDecoration: 'none' }}>{t("navbar.accueil")}</Link> {/* Lien vers la page d'accueil */}
  <span className='Path' style={{ color: 'blue' }}>&gt;</span> {/* Utilisation de span pour le symbole ">" */}
  <Link to="/pathologie" style={{ color: 'blue', textDecoration: 'none' }}>{t("Header.Path")}</Link> {/* Lien vers la page Monument */}
  <span className='Path' style={{ color: 'blue' }}>&gt;</span> {/* Utilisation de span pour le symbole ">" */}
  <Link to="/chromatique-dépot" style={{ color: 'blue', textDecoration: 'none' }}>{t("Menu.Chd")}</Link> {/* Lien vers la page Monument */}
</div>
...
export default ChromatiqueDépot;